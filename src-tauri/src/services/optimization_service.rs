...
impl OptimizationService {
    ...
    pub fn analyze_vm_performance(
        ...
        let metrics = app_state
            .metrics
            .get_historical_metrics(vm_id, start_time, end_time, None)?;
        ...
        let avg_cpu = metrics
            ...
            .map(|dp| dp.cpu_usage)
            .sum::<f64>()
            / metrics.data_points.len() as f64;
        ...
        if avg_cpu < 10.0 {
            suggestions.push(OptimizationSuggestion {
                ...
                description: format!(
                    "Average CPU usage is {:.1}% over the last {} hours",
                    avg_cpu, time_range_hours
                ),
                recommendation: "Consider reducing CPU count or consolidating workloads"
                    .to_string(),
        ...
        if max_cpu > 95.0 {
            suggestions.push(OptimizationSuggestion {
                ...
                recommendation:
                    "Monitor for CPU-intensive processes or consider increasing CPU resources"
                        .to_string(),
        ...
        let avg_memory_percent = metrics
            .data_points
            .iter()
            .map(|dp| {
        ...
        if avg_memory_percent < 20.0 {
            suggestions.push(OptimizationSuggestion {
                ...
                description: format!(
                    "Average memory usage is {:.1}% over the last {} hours",
                    avg_memory_percent, time_range_hours
                ),
                recommendation: "Consider reducing allocated memory to free up host resources"
                    .to_string(),
            ...
            suggestions.push(OptimizationSuggestion {
                ...
                vm_name: vm_name.to_string(),
                category: OptimizationCategory::Memory,
                severity: if avg_memory_percent > 95.0 {
                    OptimizationSeverity::Critical
                } else {
                    OptimizationSeverity::Warning
                },
                ...
                recommendation: "Consider increasing allocated memory or optimizing memory usage"
                    .to_string(),
                current_value: avg_memory_percent,
                threshold: 85.0,
            });
        ...
        // Analyze disk I/O
        let total_disk_read: f64 = metrics
            .data_points
            .iter()
            .map(|dp| dp.disk_read_bytes as f64)
            .sum();

        let total_disk_write: f64 = metrics
            .data_points
            .iter()
            .map(|dp| dp.disk_write_bytes as f64)
            .sum();
        let disk_read_mb_per_sec =
            (total_disk_read / (1024.0 * 1024.0)) / (time_range_hours as f64 * 3600.0);
        let disk_write_mb_per_sec =
            (total_disk_write / (1024.0 * 1024.0)) / (time_range_hours as f64 * 3600.0);
        ...
        if disk_write_mb_per_sec > 50.0 {
            suggestions.push(OptimizationSuggestion {
                ...
                description: format!(
                    "Average disk write speed is {:.2} MB/s",
                    disk_write_mb_per_sec
                ),
                recommendation: "Consider using faster storage or optimizing write operations"
                    .to_string(),
        ...
        if disk_read_mb_per_sec > 100.0 {
            suggestions.push(OptimizationSuggestion {
                ...
                description: format!(
                    "Average disk read speed is {:.2} MB/s",
        ...
        // Analyze network I/O
        let total_network_rx: f64 = metrics
            .data_points
            .iter()
            .map(|dp| dp.network_rx_bytes as f64)
            .sum();

        let total_network_tx: f64 = metrics
            .data_points
            .iter()
            .map(|dp| dp.network_tx_bytes as f64)
            .sum();
        let network_rx_mbps =
            ((total_network_rx * 8.0) / (1024.0 * 1024.0)) / (time_range_hours as f64 * 3600.0);
        let network_tx_mbps =
            ((total_network_tx * 8.0) / (1024.0 * 1024.0)) / (time_range_hours as f64 * 3600.0);
        ...
        if network_rx_mbps > 800.0 {
            suggestions.push(OptimizationSuggestion {
                ...
                description: format!(
                    "Average network receive speed is {:.2} Mbps",
                    network_rx_mbps
                ),
                recommendation:
                    "Monitor network bandwidth and consider upgrading network configuration"
                        .to_string(),
        ...
        if network_tx_mbps > 800.0 {
            suggestions.push(OptimizationSuggestion {
                ...
                description: format!(
                    "Average network transmit speed is {:.2} Mbps",
                    network_tx_mbps
                ),
                recommendation:
                    "Monitor network bandwidth and consider upgrading network configuration"
    ...
    pub fn analyze_all_vms(
        app_state: &AppState,
        connection: &impl crate::services::libvirt::ConnectionProvider,
        time_range_hours: i64,
    ) -> Result<Vec<OptimizationSuggestion>, AppError> {
        use crate::services::vm_service::VmService;

        let vms = VmService::list_vms(connection)?;
        ...
        for vm in vms {
            match Self::analyze_vm_performance(app_state, &vm.id, &vm.name, time_range_hours) {
                ...
                Err(_) => {
                    tracing::warn!("A VM performance analysis could not be completed");
                }
            }
        }
