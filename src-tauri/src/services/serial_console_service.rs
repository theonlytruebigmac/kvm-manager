...
impl Default for SerialConsoleService {
    fn default() -> Self {
        Self::new()
    }
}
...
impl SerialConsoleService {
    ...
    fn extract_serial_pty(xml: &str) -> Result<String, AppError> {
        ...
        Err(AppError::Other(
            "No serial console found for VM".to_string(),
        ))
    }
            ...
            if let Some(end_idx) = xml[start_idx..].find(&section_end) {
                ...
                if let Some(source_start) = section_content.find("<source path='") {
                    ...
                    if let Some(path_end) = section_content[path_start..].find('\'') {
                        return Some(
                            section_content[path_start..path_start + path_end].to_string(),
                        );
                    }
                }
            }
    ...
    pub fn open_connection(
        ...
        let _ = (vm_id, vm_name, pty_path);
        tracing::info!("Opening serial console");
        ...
        cmd.args(["--connect", connection_uri, "console", vm_name, "--force"]);
    ...
    pub fn close_connection(&self, vm_id: &str) -> Result<(), AppError> {
        ...
        if connections.remove(vm_id).is_some() {
            ...
            Err(AppError::Other(
                "No active serial console for this VM".to_string(),
            ))
        }
    }
