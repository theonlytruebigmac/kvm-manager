use crate::services::retention_service::RetentionPolicy;
use crate::state::app_state::AppState;
use crate::utils::error::SafeFailure;
use tauri::State;

/// Get current retention policy
#[tauri::command]
pub async fn get_retention_policy(
    state: State<'_, AppState>,
) -> Result<RetentionPolicy, SafeFailure> {
    tracing::info!("get_retention_policy command called");

    state
        .retention_service
        .load_policy()
        .map_err(SafeFailure::from)
}

/// Update retention policy
#[tauri::command]
pub async fn update_retention_policy(
    state: State<'_, AppState>,
    policy: RetentionPolicy,
) -> Result<RetentionPolicy, SafeFailure> {
    tracing::info!(
        "update_retention_policy command called: enabled={}, days={}",
        policy.enabled,
        policy.retention_days
    );

    state
        .retention_service
        .update_policy(policy)
        .map_err(SafeFailure::from)
}

/// Execute cleanup now (manual trigger)
#[tauri::command]
pub async fn execute_retention_cleanup(state: State<'_, AppState>) -> Result<usize, SafeFailure> {
    tracing::info!("execute_retention_cleanup command called");

    state
        .retention_service
        .execute_cleanup()
        .map_err(SafeFailure::from)
}
