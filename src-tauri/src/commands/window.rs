...
fn window_failure(detail: impl Into<String>) -> SafeFailure {
    AppError::WindowFailure(detail.into()).into()
}

fn open_windows(
) -> Result<std::sync::MutexGuard<'static, HashMap<String, String>>, Box<SafeFailure>> {
    OPEN_WINDOWS
        .lock()
        .map_err(|_| Box::new(window_failure("open-window registry lock is poisoned")))
}
...
pub async fn open_vm_details_window(
    ...
    {
        let windows = open_windows().map_err(|failure| *failure)?;
        if windows.contains_key(&window_label) {
            // Window already exists, just focus it
            if let Some(window) = app.get_webview_window(&window_label) {
                window
                    .set_focus()
                    .map_err(|_| window_failure("existing window cannot be focused"))?;
                return Ok(());
            } else {
    ...
    if let Some(state) = window_state::load_window_state(app.clone(), window_label.clone()).await? {
        builder = builder
    ...
    let window = builder
        .build()
        .map_err(|_| window_failure("requested window cannot be created"))?;

    // Track the window
    open_windows()
        .map_err(|failure| *failure)?
        .insert(window_label.clone(), vm_id.clone());

    // Listen for window close to remove from tracking
    let window_label_clone = window_label.clone();
    window.on_window_event(move |event| {
        if let tauri::WindowEvent::Destroyed = event {
            if let Ok(mut windows) = open_windows() {
...
#[tauri::command]
pub async fn close_vm_windows(app: AppHandle, vm_id: String) -> Result<(), SafeFailure> {
    ...
    for window_label in windows_to_close {
        if let Some(window) = app.get_webview_window(&window_label) {
            window
                .close()
                .map_err(|_| window_failure("window cannot be closed"))?;
        }
        open_windows()
            .map_err(|failure| *failure)?
            .remove(&window_label);
...
#[tauri::command]
pub async fn get_open_windows() -> Result<Vec<String>, SafeFailure> {
    let windows = open_windows().map_err(|failure| *failure)?;
...
#[tauri::command]
pub async fn close_window(app: AppHandle, window_label: String) -> Result<(), SafeFailure> {
    if let Some(window) = app.get_webview_window(&window_label) {
        window
            .close()
            .map_err(|_| window_failure("window cannot be closed"))?;
